...
def euler055():
    ...
    for i in range(10000):
       # print("[{}]".format(i))
        iterado=i
        # Las 50 iteraciones dentro de cada numero
        for j in range(50):
            #print("({}) {}+{}={}".format(j+1, iterado,reverse(iterado), iterado + reverse(iterado)))
            iterado = iterado + reverse(iterado)

            if is_palindrome(iterado):
            #    print("    -> Palindromo:",iterado)
                break
            if j == 49:
                lychrel_number += 1
                #print("LYCHREL NUMBER!!!",i)

    return lychrel_number
